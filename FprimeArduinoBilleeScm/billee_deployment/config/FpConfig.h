...
// ----------------------------------------------------------------------
// Every size/buffer/handle constant the fprime-arduino-deployment-cookiecutter
// template put in this file as a plain #define is ALSO declared as an FPP
// `constant` in F' core (lib/fprime/default/config/{FpConstants,PlatformCfg}.fpp),
// autocoded into a real enum pulled in by Fw/FPrimeBasicTypes.hpp - so
// defining it here too made the preprocessor blindly substitute this
// header's value into the enum's own initializer wherever both headers
// landed in the same translation unit (e.g.
// "enum { FW_COM_BUFFER_MAX_SIZE = 512 }" became "enum { 128 = 512 }"), a
// hard syntax error. An #ifndef guard can't fix this either: an enumerator
// isn't a macro, so it's invisible to #ifndef regardless of include order.
// This is a standing cookiecutter-template bug, independent of F' version
// or target board (confirmed against both v4.3.0 and v4.1.1).
//
// All of them are simply removed below rather than re-added as project
// overrides: this deployment targets Teensy 4.1 (~1MB RAM), where the
// framework's own default sizes need no shrinking the way an 8KB-SRAM AVR
// target would have required. If a specific value ever needs to differ from
// the framework default, override it via a project FpConstants.fpp/
// PlatformCfg.fpp CONFIGURATION_OVERRIDES (see billee_deployment/config/CMakeLists.txt),
// not by re-adding a macro here.
// ----------------------------------------------------------------------
