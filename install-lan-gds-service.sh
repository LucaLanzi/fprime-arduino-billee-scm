...
#
# After: attach with `screen -r billee-scm-lan-gds` (Ctrl-A D to detach),
#        follow logs with `journalctl -u billee-scm-lan-gds -f`.
#
# Service name is billee-scm-lan-gds (not billee-lan-gds) so this can run
# alongside fprime-billee-rcm's own LAN GDS service on the same Jetson
# without colliding - see lan_uart_gds.sh for the matching port split
# (5001 here, 5000 for fprime-billee-rcm).
set -euo pipefail

SERVICE_NAME="billee-scm-lan-gds"
...
cat > "$UNIT_PATH" <<EOF
[Unit]
Description=BILLEE SCM (Teensy 4.1) LAN F Prime GDS (detached screen, auto-retry)
...
Environment=UART_DEVICE=/dev/ttyBILLEE_SCM
